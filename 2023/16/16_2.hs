...
solve input = output
  where
    ...
    lastLineStart = w * length input
    lineW = length (head input)
    startS = map (DirectionalPosition S) [w + 1 .. w + lineW]
    startN = map (DirectionalPosition N . (+ lastLineStart)) [1 .. w - 2]
    startE = map (DirectionalPosition E) [w + 1, 2 * w + 1 .. lastLineStart + 1]
    startW = map (DirectionalPosition W) [w + lineW, 2 * w + lineW .. length input * w + lineW]

    allStarts = concat [startS, startN, startE, startW]
    output = maximum (map (countEnergized layout) allStarts)
