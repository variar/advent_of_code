...
distance :: Universe -> Int -> Int -> Int -> Int
distance universe expansion g1 g2 = abs(rowG1 - rowG2) + abs(columnG1 - columnG2) + (expansion-1)*length emptyRowsBetween + (expansion-1)*length emptyColumnsBetween
  where
...
allDistance :: Universe -> Int -> [Int] -> Int
allDistance universe expansion [] = 0
allDistance universe expansion galaxies = d + allDistance universe expansion otherG
  where 
    headG = head galaxies
    otherG = tail galaxies
    d 
      | null otherG = 0
      | otherwise = sum $ map (distance universe expansion headG) otherG

solve input = output
  where
    universe = parseUniverse input
    (w, galaxies, emptyRows, emptyColumns) = universe
    output = allDistance universe 1000000 galaxies