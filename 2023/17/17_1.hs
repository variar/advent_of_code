...
neighbors (Graph w h _) cur = allowedMoves
  where
    ...
    ifNotTooStraight (DStep _ c _ _) = c <= 3
    allowedMoves = filter ifNotTooStraight $ map (addMovesInSameDirection cur) movesForward
...
recordVisited g pos (SearchState toVisit visited heatLossMap) = SearchState nextToVisit nextVisited heatLossMap
  where
    ...
    allVisitedStates = map makeVisitedStep [nowS .. 3]
...
minHeat g = minimum (map snd heatLosses)
  where
    ...
    isTarget (n, _) = stepX n == stepX target && stepY n == stepY target
